...
def get_set_cover_submodular_order(S, metric, threshold):
    print(f'Computing set cover submodular order for {metric}...')
    start = time.time()

    F = SetCover(S, threshold)
    reps, _ = lazy_greedy_heap(F, np.arange(S.shape[0]), S.shape[0])
    cutoff = S.shape[0]
    for i in range(len(F.gains)):
        if F.gains[i] < EPS:
            cutoff = i
            break
    clusters = []
    for rep in reps[:cutoff]:
        clusters.append(list(F.sets[rep]))
    lr = np.array(F.gains[:cutoff])

    print('number of clusters:', len(clusters))
    print('time (sec) for computing set cover:', time.time() - start)
    return clusters, lr
...
def get_facility_location_submodular_order(S, B, c, smtk=0, no=0, stoch_greedy=0, weights=None):
    ...
    if smtk > 0:
        ...
        # np.save(f'./1/smtk-master1/build/smraiz', S)
        if stoch_greedy > 0:
            p = subprocess.check_output(
                f'./{no}/smtk-master{smtk}/build/smraiz -sumsize {B} \
                 -stochastic-greedy -sg-epsilon {stoch_greedy} -flnpy ./faster/tmp/{no}/{smtk}-{c}.'
                f'npy -pnpv -porder -ptime'.split())
        else:
            p = subprocess.check_output(
                f'./{no}/smtk-master{smtk}/build/smraiz -sumsize {B} \
                             -flnpy ./faster/tmp/{no}/{smtk}-{c}.npy -pnpv -porder -ptime'.split())
        s = p.decode("utf-8")
        str, end = ['([', ',])']
        ...
        print(f'... part size: {len(S)}, B: {B}', flush=True)
        V = list(range(N))
        start = time.time()
...
def save_all_orders_and_weights(folder, X, metric='l2', stoch_greedy=False, y=None, equal_num=False, outdir='.'): #todo
    ...
    def greedy(B, c):
        ...
        command = f'./{no}/smtk-master{smtk}/build/smraiz -sumsize {B} \
...
def plot_line(x=None, y=None, plot_name=None, color=None, label=None):
    '''
    Args
    - x: list of x-coordinates, defaults to range(len(y))
    - y: list of corresponding y-coordinates
    - plot_name: str, one of ['lr', 'data_loss', 'epoch_loss', 'test_loss']
    - color: str, matplotlib color
    - label: str, if given, line is plotted in solid color
        otherwise, line is plotted in faded color
    '''
    ...
    plt.figure(PLOT_NAMES.index(plot_name))
    if label is not None:
        plt.plot(x, y, '.-', label=label, c=color)
    else:
        plt.plot(x, y, alpha=0.25, c=color)


def save_plots(outdir, period, plot_names=PLOT_NAMES):
    '''
    Args
    - outdir: str, path to output directory, must already exist
    - period: int, TODO
    ...
    for idx, name in enumerate(PLOT_NAMES):
        ...
        if idx == 0:
            ...
            plt.title('learning rate per data point')
        elif idx == 1:
            plt.ylabel('loss')
            plt.xlabel(f'data point (every {period})')
            plt.title('loss per data point')
        else:
            plt.ylabel('loss')
            plt.xlabel('epoch')
            plt.title('loss per epoch')
        suffix = str(int(time.time()))
        path = os.path.join(outdir, f'{name}_plot_{suffix}.png')
        plt.savefig(path)
        plt.close()
